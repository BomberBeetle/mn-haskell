...
approxRootY :: (Float -> Float) -> Float -> Float -> Float -> Maybe Float
approxRootY f a b error
   | (f a)*(f b) > 0 = Nothing
   | (f ((a+b)/2)) == 0 = Just ((a+b)/2)
   | f ((a+b)/2) < error = Just ((a+b)/2)
   | otherwise = bifurcate f a b error

bifurcateY :: (Float -> Float) -> Float -> Float -> Float -> Maybe Float
bifurcateY f a b error
    | (f a)*f ((a+b)/2) < 0 = approxRootY f a ((a+b)/2) error
    | (f b)*f ((a+b)/2) < 0 = approxRootY f ((a+b)/2) b error
    | otherwise = Nothing